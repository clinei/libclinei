...
auto ref greatestCommonDivisor(A, B)(auto ref A a, auto ref B b)
{
	if (b == 0)
	{
		if (a > 0)
...
auto ref leastCommonMultiple(A, B)(auto ref A a, auto ref B b)
{
...
struct Rational(bool autoReduce = true, Numerator = ulong, Denominator = ulong)
{
	...
	++/
	import std.traits : isImplicitlyConvertible;
	this(Num)(auto ref Num num) if (isImplicitlyConvertible!(Num, Numerator))
	{
		opAssign(num);
	}
	this(Num)(auto ref Num num)
	...
	import std.typetuple : anySatisfy;
	this(Num, Denom)(auto ref Num num, auto ref Denom denom, bool neg = false)
	...
	this(Num, Denom)(auto ref Num num, auto ref Denom denom)
	...
	++/
	this(Other)(auto ref Other other) if (isRational!Other && isCompatibleRational!(typeof(this), Other))
	{
		opAssign(other);
	}
	...
	import std.traits : isFloatingPoint;
	this(Floating)(auto ref Floating floating, ulong precision = 10) if (isFloatingPoint!Floating)
	{
	...
	++/
	ref Rational forcePrecision(Denom = Denominator)(auto ref Denom denom)
	{
	...
	++/
	Rational atPrecision(Denom = Denominator)(auto ref Denom denom)
	{
		Rational t = this;
	...
	---
	++/
	auto ref CastType opCast(CastType)()
	{
	...
	++/
	bool opEquals(Other)(auto ref Other other) if (isRational!Other)
	{
	...
	bool opEquals(Other)(auto ref Other other) if (!isRational!Other && isIntegral!Other)
	{
	...
	++/
	auto opCmp(Other)(auto ref Other other) if (isRational!Other)
	{
		if (denom == 0) denom = 1;
	...
	+/
	ref Rational opAssign(Num)(auto ref Num num)
	...
	++/
	ref Rational opAssign(Other)(auto ref Other other)
	...
	++/
	ref Rational opOpAssign(string op, Other)(auto ref Other other)
		if (!isRational!Other)
	{
		static if (op == "^^")
		{
	...
	Rational opBinary(string op, Other)(auto ref Other other) const
	{
		Rational t = this;
		return t.opOpAssign!op(other);
	}
	Rational opBinaryRight(string op, Other)(auto ref Other other) const
	{
...
++/
auto rational(bool autoReduce = true, Num, Denom)(auto ref Num num, auto ref Denom denom)
    if (allSatisfy!(isIntegral, Num, Denom))
{
	return Rational!(autoReduce, ulong, ulong)(num, denom);
}
/// ditto
auto rational(bool autoReduce = true, Num)(auto ref Num num)
    if (isIntegral!Num)
{
...
++/
Rational!(autoReduce, ulong, ulong) rational(bool autoReduce = true, Floating)(auto ref Floating floating, ulong precision = 10)
...
unittest
{
	...
	auto r3 = Rational!(true, ulong, ulong)(42UL);
	assert(r3 == 42);
